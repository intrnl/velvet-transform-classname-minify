// parsel-js v1.0.2, licensed under MIT License
// https://github.com/LeaVerou/parsel

// contains a few modifications:
// - `walk` method now traverses list nodes.
// - `walk` no longer has mysterious `o` argument.
// - `walk` runs callback first before traversing.
...
function walk (node, callback, parent) {
	...
	if (node.type === 'complex') {
		walk(node.left, callback, node);
		walk(node.right, callback, node);
	}
	else if (node.type === 'compound' || node.type === 'list') {
		for (let n of node.list) {
			walk(n, callback, node);
		}
	}

	if (node.subtree) {
		walk(node.subtree, callback, node);
	}
}
